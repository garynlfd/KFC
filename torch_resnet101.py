...
class CI(torch.nn.Module):
    def __init__(self, gate_channels, flatten, reduction_ratio=16, pool_types=['avg', 'max']):
        super(CI, self).__init__()
        self.ChannelGate = ChannelGate(gate_channels)
        self.SpatialGate = SpatialGate(flatten)
    def forward(self, x):
        x_out = self.ChannelGate(x) # x_out size = 512, 7, 7
        x_out = self.SpatialGate(x_out) # x_out size = 25088(because flatten)
...
class ChannelGate(torch.nn.Module):
    ...
    def forward(self,x):
        ...
        channel_att_avg = self.CI_module(avg_pool)
        max_pool = F.max_pool2d(x, (x.size(2), x.size(3)), stride=(x.size(2), x.size(3)))
        channel_att_max = self.CI_module(max_pool)
...
class SpatialGate(nn.Module):
    ...
    def forward(self, x):
        x_compress = self.compress(x) # x_compress size = 2, 7, 7; 2 means 2 channel(max pool & avg pool)
        x_out = self.spatial(x_compress) # x_out size = 1, 7, 7
        scale = torch.sigmoid(x_out) # broadcasting
        x = x * scale
        # return x * scale
        if self.flatten_bool == True: # for CI
            return self.flatten(x)
        else: # for AttentionHead
            return x
...
class KitModel(nn.Module):
    ...
    def __init__(self, weight_file):
        ...
        self.CI_46 = CI(512, False)
        self.CI_47 = CI(512, False)
        self.CI_48 = CI(512, False)
    ...
    def forward(self, x):
        ...
        stage4_unit1_bn3 = self.CI_46(stage4_unit1_bn3)
        plus46          = stage4_unit1_bn3 + stage4_unit1_sc
        # plus46 = self.CI_46(plus46)
        ...
        stage4_unit2_bn3 = self.CI_47(stage4_unit2_bn3)
        plus47          = stage4_unit2_bn3 + plus46
        # plus47 = self.CI_47(plus47)
        ...
        stage4_unit3_bn3 = self.CI_48(stage4_unit3_bn3)
        plus48          = stage4_unit3_bn3 + plus47
        # plus48 = self.CI_48(plus48)
        bn1             = self.bn1(plus48)
        ...
        # return fc1
        return fc1, plus48
        # return fc1, plus20, plus30, plus45
